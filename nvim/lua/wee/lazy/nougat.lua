return {
    ...
    config = function()
        ...
        -- Color helper to ensure it matches your theme
        ...
        stl:add_item(nut.git.status.create({
            hl = { bg = color.bg1 },
            content = {
                nut.git.status.count("added", { hl = { fg = color.green }, prefix = " +" }),
        ...
        -- Diagnostic items
        local severities = {
            { sev = vim.diagnostic.severity.ERROR, col = color.red },
            { sev = vim.diagnostic.severity.WARN, col = color.yellow },
            { sev = vim.diagnostic.severity.INFO, col = color.blue },
            { sev = vim.diagnostic.severity.HINT, col = color.green },
        }

        for _, s in ipairs(severities) do
            stl:add_item(nut.buf.diagnostic_count({
                hl = { bg = s.col, fg = color.bg },
                sep_left = sep.left_chevron_solid(true),
                prefix = " ",
                suffix = " ",
                config = { severity = s.sev },
            }))
        end
